...
angular
  .module('users')
  .controller('UserController', ['User', 'Careers', 'Levels', 'UsersSrv', 'ModalSrv', '$state', '$scope', 'NotificationsSrv', 'ROLES', 'TEACHER_STATUS', 'TEACHER_ROLE', 'TEACHER_MODALITY', 'COURSES_COLORS', 'COURSES_LOGOS', 'DAYS', '$rootScope',
    function(User, Careers, Levels, UsersSrv, ModalSrv, $state, $scope, NotificationsSrv, ROLES, TEACHER_STATUS, TEACHER_ROLE, TEACHER_MODALITY, COURSES_COLORS, COURSES_LOGOS, DAYS, $rootScope) {
      var vm = this;
      vm.roles = ROLES;
      ...
      vm.onChangeRole = function() {
        console.log(123);
        vm.isTeacher = isTeacher(vm.UserTmp.role);
        vm.isStudent = isStudent(vm.UserTmp.role);
      };

      vm.onClickCanTeach = function(e) {
        e.preventDefault();

        if (vm.UserTmp.profile.canTeach) {
          var alreadyCan = vm.UserTmp.profile.canTeach.find(function(item) {
            return item === vm.id_level;
          });
          if (!alreadyCan) {
            vm.UserTmp.profile.canTeach.push(vm.id_level);
          }
        } else {
          vm.UserTmp.profile.canTeach = [];
          vm.UserTmp.profile.canTeach.push(vm.id_level);
        }

        console.log(vm.UserTmp.profile.canTeach);
      };

      vm.onClickRemoveCanTeach = function(e, levelId) {
        e.preventDefault();
        vm.UserTmp.profile.canTeach.splice(vm.UserTmp.profile.canTeach.indexOf(levelId), 1);
      };

      vm.onClickOnCareer = function(e) {
        e.preventDefault();

        if (vm.UserTmp.profile.onCareers) {
          var alreadyCan = vm.UserTmp.profile.onCareers.find(function(item) {
            return item === vm.id_career;
          });
          if (!alreadyCan) {
            vm.UserTmp.profile.onCareers.push(vm.id_career);
          }
        } else {
          vm.UserTmp.profile.onCareers = [];
          vm.UserTmp.profile.onCareers.push(vm.id_career);
        }

        console.log(vm.UserTmp.profile.assignedCareer);
      };

      vm.onClickRemoveOnCareer = function(e, careerId) {
        e.preventDefault();
        vm.UserTmp.profile.onCareers.splice(vm.UserTmp.profile.onCareers.indexOf(careerId), 1);
      };
      ...
      vm.onClickUnblock = function() {
        vm.UserTmp.enable = true;
        vm.onSubmit();
      };

      vm.onSubmit = function() {
        // if (isTeacher(vm.UserTmp.role) && vm.UserTmp.profile && (!vm.UserTmp.profile.canTeach || !vm.UserTmp.profile.canTeach.length)) {
        //   NotificationsSrv.error({
